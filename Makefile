# This Makefile is for the Net::WebSocket::Server extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Net/WebSocket/Server.pm]
...
#     NAME => q[Net::WebSocket::Server]
#     PL_FILES => {  }
#     PREREQ_PM => { Protocol::WebSocket=>q[0.12], Test::More=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Net/WebSocket/Server.pm]
#     clean => { FILES=>q[Net-WebSocket-Server-*] }
...
NAME_SYM = Net_WebSocket_Server
VERSION = 0.003004
VERSION_MACRO = VERSION
VERSION_SYM = 0_003004
...
MAKE = make
FULLEXT = Net/WebSocket/Server
BASEEXT = Server
PARENT_NAME = Net::WebSocket
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Net/WebSocket/Server.pm \
	lib/Net/WebSocket/Server/Connection.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Net/WebSocket
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Net/WebSocket
...
TO_INST_PM = lib/Net/WebSocket/Server.pm \
	lib/Net/WebSocket/Server/Connection.pm

PM_TO_BLIB = lib/Net/WebSocket/Server.pm \
...
DISTNAME = Net-WebSocket-Server
DISTVNAME = Net-WebSocket-Server-0.003004
...
manifypods : pure_all  \
	lib/Net/WebSocket/Server.pm \
	lib/Net/WebSocket/Server/Connection.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Net/WebSocket/Server.pm $(INST_MAN3DIR)/Net::WebSocket::Server.$(MAN3EXT) \
	  lib/Net/WebSocket/Server/Connection.pm $(INST_MAN3DIR)/Net::WebSocket::Server::Connection.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A straightforward Perl WebSocket server with minimal dependencies. '\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Eric Wastl <topaz@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Net-WebSocket-Server' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "A straightforward Perl WebSocket server with minimal dependencies. ",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Net-WebSocket-Server",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Protocol::WebSocket" : "0.12",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>A straightforward Perl WebSocket server with minimal dependencies. </ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Eric Wastl &lt;topaz@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Protocol::WebSocket" VERSION="0.12" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Net/WebSocket/Server.pm blib/lib/Net/WebSocket/Server.pm \
	  lib/Net/WebSocket/Server/Connection.pm blib/lib/Net/WebSocket/Server/Connection.pm 
